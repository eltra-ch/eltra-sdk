...
namespace EltraCloudContracts.ObjectDictionary.Xdd.DeviceDescription.Profiles.Application.DataTypes
{
    class XddStructDataType : XddDataTypeReference
    {
        ...
        private List<XddVarDeclaration> _varDeclarations;

        public List<XddVarDeclaration> VarDeclarations { get => _varDeclarations ?? (_varDeclarations = new List<XddVarDeclaration>()); }

        #endregion

        #region Constructors

        public XddStructDataType(XmlNode source, XddDataTypeList dataTypeList)
        {
            _source = source;
        ...
        public override bool Parse()
        {
            ...
            foreach (XmlNode childNode in _source.ChildNodes)
            {
                ...
                {
                    var varDeclaration = new XddVarDeclaration(_dataTypeList);
