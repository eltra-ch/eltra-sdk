...
using EltraNavigo.Views.RelayControl.ObjectDictionary;
...
namespace EltraNavigo.Views.RelayControl
{
    public class RelayControlViewModel : RelayToolViewModel
    {
        ...
        private async Task RegisterParameterUpdate()
        {
            if (Vcs != null)
            {
                for (int channelIndex = 0; channelIndex < _channelCount; channelIndex++)
                {
                    _parameters[channelIndex] = await Vcs.GetParameter($"PARAM_RelayState_{channelIndex + 1}");
